...
def parse_trigger_file_and_write(root):
        ...
        most_likely_2 = most_likely_2_lookup[tmp]
        ...
        lc_file = (
            root
            .find(".//Param[@name='LightCurve_URL']")
            .attrib["value"]
        )

        # now we want to store the folder directory

        main_ftp_directory = os.path.join(
        ...
        out_file_writer = GBMTriggerFile(name=burst_name,
                                         ra=ra,
                                         dec=dec,
                                         radius=radius,
                                         uri=uri,
